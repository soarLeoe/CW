/*
 * algorithm.c — Heart rate / SpO2 calculation algorithm
 * Ported from Maxim MAXREFDES117# algorithm.cpp to plain C.
 *
 * Original: Copyright (C) 2016 Maxim Integrated Products, Inc.
 * MIT License — see original source for full text.
 */
...
/* -----------------------------------------------------------------------
 * Working buffers (file-scope, same as original `static` globals)
 * --------------------------------------------------------------------- */

static int32_t an_dx[BUFFER_SIZE - MA4_SIZE];   /* delta           */
static int32_t an_x[BUFFER_SIZE];               /* IR  working buf */
static int32_t an_y[BUFFER_SIZE];               /* RED working buf */
                                            ...
                                            int8_t   *pch_hr_valid)
{
    ...
    /* 2-point moving average on an_dx */
    for (k = 0; k < BUFFER_SIZE - MA4_SIZE - 2; k++)
        an_dx[k] = (an_dx[k] + an_dx[k + 1]) / 2;

    /* Hamming window — flip waveform so valley detection = peak detection */
    ...
    /* peak location = index of sharpest point of raw signal (flipped) */
    ...
    if (n_npks >= 2) {
        ...
        *pn_heart_rate = (int32_t)(6000 / n_peak_interval_sum);   /* BPM */
        *pch_hr_valid = 1;
    } else {
        *pn_heart_rate = -999;
        *pch_hr_valid = 0;
    ...
    /* raw values: RED(=y) and IR(=x) */
    for (k = 0; k < n_ir_buffer_length; k++) {
        an_x[k] = pun_ir_buffer[k];
        an_y[k] = pun_red_buffer[k];
    }

    /* find precise minimum near an_ir_valley_locs */
    ...
    /* find AC/DC ratio between valley locations for SpO2 */
    n_ratio_average = 0;
    n_i_ratio_count = 0;
